...
public class PersonPersistenceTest extends AbstractPersistenceTest {
    ...
    public void userCreatedIsAutomaticallyFilledIn() throws Exception {
        Person p = new Person();
        p.setFirstName("Kurt");
        p.setLastName("Cobain");
        p.setBirthDate(new Date());
        em.persist(p);
        flushAndClear();

        Person pp = em.find(Person.class, p.getId());
        assertEquals(UserUtils.currentUser(), pp.getUserCreated());
    }

    @Test
    public void userModifiedIsAutomaticallyFilledIn() throws Exception {
        Person p = em.find(Person.class, 2000L);
        p.setFirstName("Atkins");
        flushAndClear();
        Person pp = em.find(Person.class, p.getId());
        assertEquals(UserUtils.currentUser(), pp.getUserModified());
    }
}
